...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($nombre) || empty($correo) || empty($usuario) || empty($contrasena_actual)) {
        header("Location: ../admin/perfil.php?error=Todos los campos obligatorios deben estar llenos");
        exit();
    }

    // Confirmar nueva contraseña si se proporcionó
    if (!empty($contrasena_nueva) && $contrasena_nueva !== $confirmar_contrasena) {
        header("Location: ../admin/perfil.php?error=La nueva contraseña no coincide con la confirmación");
        exit();
    }
    ...
    if (!$stmt->fetch()) {
        header("Location: ../admin/perfil.php?error=Usuario no encontrado");
        exit();
    }
    $stmt->close();

    if (!password_verify($contrasena_actual, $hash_guardado)) {
        header("Location: ../admin/perfil.php?error=Contraseña actual incorrecta");
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../admin/panel.php?exito=Perfil actualizado correctamente");
    } else {
        header("Location: ../admin/perfil.php?error=" . urlencode("Error al actualizar: " . $stmt->error));
    }
